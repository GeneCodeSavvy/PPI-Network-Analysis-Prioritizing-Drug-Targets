fastCGP <- function(snp2gene_file, snp_chr_pos_p_file, output_dir, genes2compute_file) {
  ...
  fastCGP_cir_info <- function(snp2gene, snp_chr_pos_p) {
    ...
    snps_pos_table <- numeric(length = snps_count)  # Initialize snps_pos_table as a numeric vector
    snp_names <- snp_chr_pos_p$SNP
    unique_snp_names <- unique(snp_names)
    names(snps_pos_table) <- unique_snp_names  # Assign unique SNP names to snps_pos_table
    
    # Now populate the positions based on SNP names
    ordered_snp_pos <- snp_chr_pos_p[order(snp_chr_pos_p$chr, snp_chr_pos_p$pos), ]
    ordered_snp_names <- ordered_snp_pos$SNP
    ordered_snp_positions <- seq_along(ordered_snp_names)
    
    matched_positions <- match(ordered_snp_names, unique_snp_names)  # Match SNP names to positions in snps_pos_table
    snps_pos_table[matched_positions] <- ordered_snp_positions
    ...
    gene_snps_poses = tapply(snps_pos_table[snp2gene$SNP], snp2gene$gene, function(v) {
      return(v)
    })
    if (any(is.na(unlist(gene_snps_poses, recursive = TRUE)))) {
      cat('NA information from SNP table again!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
      return(0)
    }
    ...
    snp2gene_p = snp2gene
    snp2gene_p$p = snps_p[snp2gene$SNP]
  ...
  ##read files:
  snp2gene = utils::read.table(snp2gene_file, header = TRUE, stringsAsFactors = FALSE, comment.char = '', sep = ",")
  snp_chr_pos_p = utils::read.table(snp_chr_pos_p_file, header = TRUE, stringsAsFactors = FALSE, comment.char = '', sep = ",")
  ...
  if (missing(genes2compute_file)) genes2compute = genes
    ...
    genes2compute = utils::read.table(genes2compute_file, header = TRUE, stringsAsFactors = FALSE, comment.char = '')
  ...
  for (gene in genes2compute) {
    ...
    if (gene_len == 1) {
      genes_p[[gene]] = cir_info$gene_minP[gene]
      next
    } ##if the gene only contains one SNP, the p-value will be that SNP p-value
    
    thresh = cir_info$gene_minrk[gene] ##Test statistics //
    sig_pos = sig_pos_ini[which(sig_pos_ini[, 'rk'] <= thresh), 'pos'] ## CONFIRMED
    if (length(sig_pos) == 1) {
      genes_p[[gene]] = (gene_len + 1) / (table_len + 1)
      next
    } ##only extreme cases, that a gene contains the best snp in all_snps
    
    gaps = c(diff(sig_pos) - 1, (table_len - sig_pos[length(sig_pos)]) + (sig_pos[1] - 1)) ##CONFIRMED
    big_gaps = gaps[gaps >= gene_len]
    gap_width = big_gaps - gene_len + 1 ##CONFIRMED
  ...
  gene_names <- genes_p_frame[, "gene"]
  computed_genes <- file.path(output_dir, "/computed_genes.tsv")
  computed_gene_p_values <- file.path(output_dir, "/computed_gene_p_values.tsv")
  
  # Write the column to a .tsv file
  utils::write.table(gene_names, file = computed_genes, sep = "\t", row.names = FALSE, quote = FALSE)
  utils::write.table(genes_p_frame, file = computed_gene_p_values, row.names = FALSE, sep = '\t', quote = FALSE)
