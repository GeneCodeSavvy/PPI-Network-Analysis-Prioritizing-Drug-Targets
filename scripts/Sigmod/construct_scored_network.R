...
library(igraph)
	...
	construct_scored_net <- function( gene_network_data, interaction_indices, weight_index=NULL, gene_scores, output )
	{
		...
		net <- graph_from_data_frame(gene_network_data, directed = FALSE, vertices = NULL)
		...
		logfile = file.path(output, '/log.txt')
		...
		filename = file.path(output, '/constructed_scored_net.Rdata')
