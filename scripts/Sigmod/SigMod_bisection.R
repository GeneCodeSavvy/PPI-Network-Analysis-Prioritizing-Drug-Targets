	...
	SigMod_bisection <- function( net, lambda_min=0, lambda_max=1, eta_max=1E7, size_diff=1, size_min=0, nmax=300, maxjump=30, epsilon=1E-5, cp=1000, silent=1, output_dir = "./" )
	{
		...
		file_compute_details = file.path(output_dir, '/SigMod_computation_details.Rdata')
		save(info, file=file_compute_details)
		obtain_results( info, net, output_dir )
		return( info )
	}
	
	obtain_results <- function( SigMod_computation_details, net, output_dir = './output/' )
	{
		...
		# Specify the output file paths within the specified output directory
		selected_genes_output_path <- paste(output_dir, 'selected_genes.tab', sep = '/')
		selected_genes_next_output_path <- paste(output_dir, 'selected_genes_next.tab', sep = '/')
		
		write.table(selected_genes_info, file = selected_genes_output_path, 
		...
		pdf(file.path(output_dir, '/selected_module.pdf'))
		par(mar=c(0,0,2,0))
		set.seed(1)
		...
		write.table(selected_genes_info, file = selected_genes_next_output_path, 
		            quote = FALSE, row.names = FALSE, col.names = TRUE)
		
		# plot the selected module
		main = paste('Number of nodes:', vcount(selected_module), '\n', 'Number of interactions:', ecount(selected_module))
		pdf(file.path(output_dir, '/selected_module_next.pdf'))
		par(mar=c(0,0,2,0))
		set.seed(1)
		plot( selected_module, vertex.size = 5, vertex.label=NA, main=main)
		set.seed(1)
